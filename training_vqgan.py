...
class TrainVQGAN:
    ...
    def __init__(self, args):
         ...
         self.disciminator = Discriminator(args).to(device = args.device)
         self.disciminator.apply(weights_init)
         self.perceptual_loss = LPIPS.eval().to(device = args.device)
         self.opt_vq, self.opt_disc = self.configure_optimizers(args)
         
         self.perpare_training()
         self.train(args)
    ...
    def configure_optimizers(self, args):
        ...
        opt_disc = torch.optim.Adam(self.disciminator.parameters(), lr = lr, eps = 1e-08, betas = (args.beta1, args.beta2)) 
        ...
        for epoch in range(args.epoch):
            with tqdm(range(len(train_dataset))) as pbar:
                for i, imgs in zip(pbar, train_dataset):
                    ...
                    disc_real = self.disciminator(imgs)
                    disc_fake  =  self.disciminator(decoded_images)
                    ...
                    vq_loss = perceptual_rec_loss + q_loss + disc_factor * lamda * g_loss
                    ...
                    self.opt_vq_zero_grad()
                    vq_loss.backward(retain_graph = True)
                    
                    self.opt_disc_zero_grad()
                    gan_loss.backward()
...
if __name__ == '__main__':
    ...
    args.dataset_path = r"/home/suresh/Desktop/NewProject/archive/images"
